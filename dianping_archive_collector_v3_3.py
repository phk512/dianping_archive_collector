...
APP_NAME = "Dianping Archive Collector V3.3"
APP_VERSION = "3.3"
...
def list_page_url(base_url, page):
    """
    把列表页 / 搜索结果页地址改写成第 N 页的地址。

    点评的翻页形式有两种（实测）：`/shanghai/ch10/p2`（路径式）与 `?pg=2`（参数式）。
    """
    ...
    if page <= 1:
        ...
        url = re.sub(r"([?&])pg=\d+", r"\1", url)
        return url.rstrip("?&")

    if re.search(r"/p\d+(?=$|[?#])", url):
        return re.sub(r"/p\d+(?=$|[?#])", f"/p{page}", url)
    if re.search(r"([?&])pg=\d+", url):
        return re.sub(r"([&?])pg=\d+", rf"\1pg={page}", url)
    if "?" in url:
        head, _, tail = url.partition("?")
        return f"{head.rstrip('/')}/p{page}?{tail}"
    return f"{url.rstrip('/')}/p{page}"
...
# 点评的列表页地址是这样拼的：
#
#     https://www.dianping.com/{城市拼音}/ch{一级分类}[g{二级分类}][r{商圈}][o{排序}][/p{页码}]
#
# 实例（用户提供 + 实测）：
#     /shanghai/ch10        -> 上海 · 餐饮（美食）
#     /shanghai/ch10/o11    -> 上海 · 餐饮 · 按「评价最多」排序
#     /shanghai/ch10/p2     -> 第 2 页
...
def parse_list_url(url):
    ...
    rest = components[1:]
    if rest and re.fullmatch(r"p\d+", rest[-1], re.I):
        result["page"] = int(rest[-1][1:])
        rest = rest[:-1]
...
def build_list_url(city, category="", group="", region="", price="",
                   sort="", page=1):
    """
    拼出列表页地址：/{城市}/[ch{n}][/g{n}][/r{n}][/o{n}][/p{页码}]
    ...
    if not codes or not codes[0].startswith("ch"):
        ...
        codes.insert(0, "ch10")
    url = "https://www.dianping.com/" + pinyin + "/" + "/".join(codes)
    try:
        page = int(page)
    except (TypeError, ValueError):
        page = 1
    ...
    def discover_from_url(self, url, max_pages=3, source="列表页", **filters):
        ...
        for page in range(1, max_pages + 1):
            if not self.check_control():
                ...
                break
            page_url = list_page_url(url, page)
            html, error = self.fetch_list_page(page_url)
            if error:
                self.logger(f"[DISCOVER] 第 {page} 页失败：{error}")
                if page == 1:
    ...
    def discover_city(self, city, category="", sort="", max_pages=2,
        ...
        for option in category_options:
            ...
            for page in range(1, max_pages + 1):
                if not self.check_control():
                    break
                page_url = list_page_url(category_url, page)
                ...
                if page_error:
                    self.logger(f"[扫描] {label} 第 {page} 页失败：{page_error}")
                    break
...
列表页地址语法（可以自己拼，也可以从页面里学）：
  /{{城市拼音}}/ch{{一级分类}}[/g{{二级分类}}][/r{{商圈}}][/o{{排序}}][/p{{页码}}]
...
def cli_selftest():
    ...
    if not BS4_AVAILABLE:
        print(f"✗ beautifulsoup4 缺失：{BS4_ERROR}")
