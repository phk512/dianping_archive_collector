...
APP_NAME = "Dianping Archive Collector V3.2"
APP_VERSION = "3.2"
            ...
            if soup is not None:
                for selector in self.LIST_CARD_SELECTORS:
                    ...
                    if not nodes:
                        continue
                    for node in nodes:
                        anchor = node.find("a", href=self._SHOP_HREF_RE)
                        if anchor is None:
                            continue
                        match = SHOP_ID_RE.search(anchor.get("href", ""))
                        if not match:
                            continue
                        shop_id = match.group(1)
                        if (
                        ...
                        name = clean_display_name(
                            anchor.get_text(" ", strip=True), max_len=60
                        )
                        text = self._card_text(node)
                        cards.append((shop_id, name, self._age_hint(text)))
                    if cards:
                        return cards
...
class DianpingArchiveGUI:
    ...
    def create_discover_ui(self):
        ...
        filters.grid(row=5, column=0, columnspan=4, sticky="ew", pady=8)
